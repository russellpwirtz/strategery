...
public enum EnemyState implements State<EnemyAgent> {
    ...
    MOVE_UP() {
        ...
        public void update(EnemyAgent entity) {
            if (entity.enemyComponent.isFixed()) {
                entity.enemyComponent.currentState = EnemyComponent.FIXED;
                return;
            }

            entity.enemyComponent.currentState = EnemyComponent.MOVE_UP;

            Body body = entity.enemyComponent.getBody();
            ...
            if (checkHitWall(entity, EnemyComponent.MOVE_UP)) {
                changeState(entity, getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_DOWN)));
                return;
            }

            if (entity.timer > 0.5f && inPosition(entity, 0.05f)) {
                entity.timer = 0;
                int newState = getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_DOWN));
                if (newState != entity.enemyComponent.currentState) {
                    changeState(entity, newState);
                    return;
                }
            }

//            if (entity.enemyComponent.weaken) {
//                entity.enemyComponent.currentState = EnemyComponent.ESCAPE;
//                if (entity.enemyComponent.hp <= 0 && inPosition(entity, 0.1f)) {
...
//            }
    ...
    MOVE_DOWN() {
        ...
        public void update(EnemyAgent entity) {
            ...
            entity.enemyComponent.currentState = EnemyComponent.MOVE_DOWN;
            ...
            if (checkHitWall(entity, EnemyComponent.MOVE_DOWN)) {
                changeState(entity, getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_UP)));
                return;
            }

            if (entity.timer > 0.5f && inPosition(entity, 0.05f)) {
                entity.timer = 0;
                int newState = getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_UP));
                if (newState != entity.enemyComponent.currentState) {
                    changeState(entity, newState);
                    return;
                }
...
//                }
    ...
    MOVE_LEFT() {
        ...
        public void update(EnemyAgent entity) {
            ...
            entity.enemyComponent.currentState = EnemyComponent.MOVE_LEFT;
            ...
            if (checkHitWall(entity, EnemyComponent.MOVE_LEFT)) {
                changeState(entity, getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_RIGHT)));
                return;
            }

            if (entity.timer > 0.5f && inPosition(entity, 0.05f)) {
                entity.timer = 0;
                int newState = getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_RIGHT));
                if (newState != entity.enemyComponent.currentState) {
                    changeState(entity, newState);
                    return;
                }
...
//                }
    ...
    MOVE_RIGHT() {
        ...
        public void update(EnemyAgent entity) {
            ...
            entity.enemyComponent.currentState = EnemyComponent.MOVE_RIGHT;
            ...
            if (checkHitWall(entity, EnemyComponent.MOVE_RIGHT)) {
                changeState(entity, getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_LEFT)));
                return;
            }

            if (entity.timer > 0.5f && inPosition(entity, 0.05f)) {
                entity.timer = 0;
                int newState = getRandomDirectionChoice(getDirectionChoices(entity, EnemyComponent.MOVE_LEFT));
                if (newState != entity.enemyComponent.currentState) {
                    changeState(entity, newState);
                    return;
                }
    ...
    PURSUE() {
        ...
        public void update(EnemyAgent entity) {
            ...
            if (!nearPlayer(entity, PURSUE_RADIUS) && inPosition(entity, 0.1f)) {
                changeState(entity, entity.enemyComponent.currentState);
                return;
            }
...
//            if (!entity.enemyComponent.weaken && inPosition(entity, 0.1f)) {
//                entity.stateMachine.changeState(PURSUE);
//                return;
//            }
    ...
    DIE() {
        ...
        public void update(EnemyAgent entity
            ...
            entity.enemyComponent.currentState = EnemyComponent.DIE;
            ...
            if (entity.nextNode == null || entity.getPosition().dst2(GameManager.instance.enemySpawnPos) < 0.04f) {
                // no path found or reach target
                entity.enemyComponent.getBody().setTransform(GameManager.instance.enemySpawnPos, 0);
    ...
    protected boolean nearPlayer(EnemyAgent entity, float distance) {
    ...
    protected boolean inPosition(EnemyAgent entity, float radius) {
    ...
    protected void changeState(EnemyAgent entity, int state) {
        switch (state) {
            case EnemyComponent.MOVE_UP: // UP
                entity.stateMachine.changeState(MOVE_UP);
                break;
            case EnemyComponent.MOVE_DOWN: // DOWN
                entity.stateMachine.changeState(MOVE_DOWN);
                break;
            case EnemyComponent.MOVE_LEFT: // LEFT
                entity.stateMachine.changeState(MOVE_LEFT);
                break;
            case EnemyComponent.MOVE_RIGHT: // RIGHT
                entity.stateMachine.changeState(MOVE_RIGHT);
                break;
            case EnemyComponent.ESCAPE: // ESCAPE
                entity.stateMachine.changeState(ESCAPE);
                break;
            case EnemyComponent.DIE: // DIE
                entity.stateMachine.changeState(DIE);
                break;
            default:
                break;
    ...
    protected boolean checkHitWall(EnemyAgent entity, int state) {
        Body body = entity.enemyComponent.getBody();
        ...
        switch (state) {
            case EnemyComponent.MOVE_UP:
                tmpV2.set(tmpV1).add(0, RADIUS);
                break;
            case EnemyComponent.MOVE_DOWN:
                tmpV2.set(tmpV1).add(0, -RADIUS);
                break;
            case EnemyComponent.MOVE_LEFT:
                tmpV2.set(tmpV1).add(-RADIUS, 0);
                break;
            case EnemyComponent.MOVE_RIGHT:
                tmpV2.set(tmpV1).add(RADIUS, 0);
                break;
            default:
                tmpV2.setZero();
    ...
    protected Integer[] getDirectionChoices(EnemyAgent entity, int state) {
        Body body = entity.enemyComponent.getBody();
        World world = body.getWorld();
    ...
    public void enter(EnemyAgent entity) {
        entity.enemyComponent.getBody().setLinearVelocity(0, 0);
        if (!inPosition(entity, 0.1f)) {
            entity.enemyComponent.getBody().setTransform(tmpV1.set(MathUtils.floor(entity.getPosition().x) + 0.5f, MathUtils.floor(entity.getPosition().y) + 0.5f), 0);
        }
        entity.timer = 0;
    }
